  ...
  if (isLastCard) {
    return (
      <div className="animate-fadeIn text-center py-8">
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-2">{t('learning.flashcards.allDone')}</h3>
        <div className="flex justify-center gap-4 mb-6">
          <div className="bg-emerald-50 rounded-xl px-4 py-3 text-center">
            ...
            <p className="text-xs text-emerald-500">{t('learning.flashcards.remembered')}</p>
          </div>
          <div className="bg-amber-50 rounded-xl px-4 py-3 text-center">
            <p className="text-2xl font-bold text-amber-600">{toRepeat.size}</p>
            <p className="text-xs text-amber-500">{t('learning.flashcards.toRepeat')}</p>
          </div>
        </div>
        <button
          onClick={onComplete}
    ...
    <div className="animate-fadeIn">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-bold text-gray-900">{t('learning.flashcards.title')}</h3>
          <p className="text-xs text-gray-400">{t('learning.flashcards.tapToFlip')}</p>
        </div>
        ...
        >
          ...
          >
            <div className="text-center">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-3 font-semibold">{t('blocks.question')}</p>
              <p className="text-lg font-bold text-gray-800 leading-relaxed">{bl(currentCard.front, lang)}</p>
              <p className="text-xs text-gray-300 mt-4">{t('learning.flashcards.tapToSeeAnswer')}</p>
            </div>
          </div>
          ...
          >
            <div className="text-center">
              <p className="text-xs text-blue-400 uppercase tracking-wider mb-3 font-semibold">{t('learning.flashcards.answer')}</p>
      ...
      {isFlipped && (
        <div className="flex gap-3 justify-center animate-fadeIn">
          ...
          >
            <span>✅</span> {t('learning.flashcards.remember')}
          </button>
          <button
          ...
          >
            <span>🔄</span> {t('learning.flashcards.repeat')}
          </button>
        </div>
      )}
    </div>
