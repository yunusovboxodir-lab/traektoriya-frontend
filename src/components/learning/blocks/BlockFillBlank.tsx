...
export function BlockFillBlank({ data, accent, accentSoft, onAnswer, onReady }: Props) {
  ...
  const handleSelect = useCallback((option: string) => {
    ...
    onAnswer(correct);
    onReady();
  }, [selected, data.correctAnswer, onAnswer, onReady]);
      ...
      <div className="bg-white mx-3 rounded-2xl p-5 shadow-sm">
        <div className="text-[15px] leading-[1.8] mb-3.5">
          ...
          <span
            ...
            style={{
              borderColor: selected === null ? accent : isCorrect ? '#22c55e' : '#ef4444',
              color: selected === null ? accent : isCorrect ? '#16a34a' : '#dc2626',
            }}
          >
            {selected !== null ? data.correctAnswer : '___'}
          </span>
          {data.sentenceAfter || ''}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {data.options.map((opt) => {
            ...
            if (selected !== null) {
              if (opt === data.correctAnswer) optClass = 'border-green-500 bg-green-50 text-green-600';
              else if (opt === selected) optClass = 'border-red-500 bg-red-50 text-red-600';
            }
