...
export interface ChampionshipResponse {
  ...
  /** Период содержит демонстрационные данные (вставлены сидом, а не посчитаны). */
  is_demo?: boolean;
  /** Первый период с настоящими данными, формат YYYY-MM. */
  first_trusted_period?: string;
  /** Машиночитаемая причина пустого результата — по ней собирается текст на языке пользователя. */
  reason?: 'no_trusted_closed_period' | 'demo_period';
  /**
   * Пояснение с бэкенда, всегда по-русски. Для служебных инструментов.
   * В интерфейсе НЕ показывать: в узбекском режиме это даст русский текст —
   * ровно то, что вычищали при переводе (833 хардкода → 0).
   */
  note?: string;
}
